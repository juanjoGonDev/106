...
export function renderPlayerRadarHtml(htmlSource, rendererRevision) {
  ...
  const consumerIndex = firstPlayerConsumerIndex(sourceWithoutDirectModel);
  if (consumerIndex < 0) {
    throw new Error('Player radar HTML must load player-ui.js or player-stats.js.');
  }

  const expectedModelScript = `<script src="./player-radar-model.js?v=${revision}"></script>`;
  const lineStart = sourceWithoutDirectModel.lastIndexOf('\n', consumerIndex - 1) + 1;
  const indentation = sourceWithoutDirectModel.slice(lineStart, consumerIndex);
  const separator = isWhitespaceOnly(indentation) ? `\n${indentation}` : '';
  return `${sourceWithoutDirectModel.slice(0, consumerIndex)}${expectedModelScript}${separator}${sourceWithoutDirectModel.slice(consumerIndex)}`;
}
...
async function readRadarConsumerHtmlEntries() {
...
export async function synchronizePlayerRadarModels({ check = false } = {}) {
  const [canonicalSource, migrationFiles, htmlEntries] = await Promise.all([
    readFile(PLAYER_RADAR_MODEL_PATHS.canonical, 'utf8'),
    readdir(PLAYER_RADAR_MODEL_PATHS.migrations),
    readRadarConsumerHtmlEntries(),
  ]);
