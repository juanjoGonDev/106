...
describe('zadmin authentication and brute-force boundary', () => {
  ...
  it('enforces independent rolling IP and device limits transactionally', () => {
    const gate = functionBody(retryMigration, 'zadmin_login_gate');
  ...
  it('uses random memory-only sessions stored as hashes and bound to IP plus device', () => {
    ...
    const createSession = functionBody(migration, 'zadmin_create_session');
    const validateSession = functionBody(migration, 'zadmin_validate_session');
...
describe('zadmin data and ban authorization', () => {
  ...
  it('keeps manual operator bans separate, revocable and audited', () => {
    ...
    expect(functionBody(migration, 'zadmin_revoke_manual_ban')).not.toMatch(/delete\s+from\s+public\.game_admin_bans/i);
  });

  it('supports exactly hourly 1-24h, one week and permanent operator durations', () => {
    const createBan = functionBody(migration, 'zadmin_create_manual_ban');
  ...
  it('requires an active admin session, existing target and reason for every ban mutation', () => {
    const createBan = functionBody(migration, 'zadmin_create_manual_ban');
    const revokeBan = functionBody(migration, 'zadmin_revoke_manual_ban');
  ...
  it('extends the existing canonical ranked restriction lookup instead of bypassing policy v3', () => {
    const byNick = functionBody(migration, 'get_game_active_integrity_ban');
    const byToken = functionBody(migration, 'get_game_active_integrity_ban_by_token');
