...
test('clean player routes expose responsive overview, achievements and trophies', async ({ page }) => {
  ...
  const shareCursor = await page.locator('#sharePlayer').evaluate((element) => getComputedStyle(element).cursor);
  const tabCursor = await page.getByRole('link', { name: 'Logros' }).evaluate((element) => getComputedStyle(element).cursor);
