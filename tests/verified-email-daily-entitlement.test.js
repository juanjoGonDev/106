...
describe('authentication daily entitlement migration', () => {
  it('keeps one account-wide bonus inside the existing ceiling', () => {
    ...
    expect(sql).toContain("entitlement.entitlement_code in ( 'auth_identity_daily_attempt', 'verified_email_daily_attempt' )");
    expect(sql).toContain('public.daily_game_account_id(entitlement.account_id) = public.daily_game_account_id(p_account_id)');
    expect(sql).toContain('+ public.game_account_auth_daily_bonus(selected.account_id)');
    expect(sql).toContain('select least( 5,');
    expect(sql).toContain("'authrewardbonus', v_auth_bonus");
    expect(sql).toContain("'emailverificationbonus', v_auth_bonus");
    ...
    expect(sql).toContain('revoke all on function public.game_account_auth_daily_bonus(uuid) from public, anon, authenticated');
    expect(sql).toContain('grant execute on function public.game_account_auth_daily_bonus(uuid) to service_role');
  });
});
