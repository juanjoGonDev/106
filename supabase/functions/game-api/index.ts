...
function messageForError(error: string) {
  const messages: Record<string, string> = {
    invalid_input: 'Datos inválidos.', nick_limit: 'Este nick ya ha agotado sus intentos disponibles.',
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (action === 'profile' || action === 'public-profile' || action === 'nick-status') {
    ...
    if (action === 'finish') {
      ...
      const result = await rpc('finish_game_attempt', {
        p_challenge_id: challengeId, p_client_elapsed_ms: clientElapsedMs,
        p_device_hash: deviceHash, p_ip_hash: ipHash, p_client_signals: normalizeSignals(body.clientSignals),
      });
      ...
      const achievement = {
        ...
        isWorldRecord: result.attempt?.verified === true && Number(profile.globalRankBest) === 1 && Number(result.attempt?.differenceMs) === Number(profile.bestDifferenceMs),
    ...
    if (action === 'create-league') {
      ...
      return safeResult(origin, await rpc('create_game_league', { p_name: name, p_owner_nick_key: key, p_device_hash: deviceHash }), 201);
    }
