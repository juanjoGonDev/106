...
begin
  ...
  where code = upper(trim(p_code)) or public_id = upper(trim(p_code));
...
select coalesce(jsonb_agg(
  jsonb_build_object(
    ...
    'participantCount', (select count(*)::integer from public.game_league_members member where member.league_id = league.id),
