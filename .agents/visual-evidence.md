...
3. **Animated GIF** — the complete interaction, beginning before the changed state appears and ending after its animation or transition finishes.

Static screenshots and GIF frames must capture the page, not an isolated locator. The surrounding UI is part of the evidence because it proves positioning, overlap, responsive behavior and viewport containment.

## Quality rules

- Capture from the current PR head only.
- Use Playwright project viewports and native screenshot resolution.
- Use lossless PNG source frames.
...
- GIF conversion must preserve the complete frame, keep aspect ratio, use Lanczos scaling and a full 256-color palette.
...
The command clears and recreates `.tmp/pr-previews/`, runs the responsive browser suite, captures complete Desktop/Mobile screenshots and frame sequences, and encodes every frame directory into a looping GIF.
...
.tmp/pr-previews/frames/example-desktop/*.png
.tmp/pr-previews/frames/example-mobile/*.png
```
...
1. Attach the generated PNG/GIF files directly to the pull request.
...
3. Include the GIF and download links in the final user-facing report whenever the work is performed through an agent.
