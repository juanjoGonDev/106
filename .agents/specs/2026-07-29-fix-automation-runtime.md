...
- Dependabot run `30449377010` failed while creating `requires-manual-qa` because `gh label create` had no repository context and the job intentionally performs no checkout.
...
- Code review identified missing fork guards and an unnecessary `contents: read` permission in cache maintenance.
...
- YAML parsed locally with a non-coercing loader.
- Actions remain pinned by immutable SHA.
- Runtime behavior will be validated by pull-request CI and the next Dependabot event after merge.
