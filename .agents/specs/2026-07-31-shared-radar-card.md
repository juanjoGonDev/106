...
Implementation in progress on `agent/fix-shared-radar-card`. No merge, deployment, release or production migration has been performed.
...
- The `player-share` Edge Function calls the older `get_game_public_profile` RPC, so its card renderer does not necessarily receive the same lifetime/daily profile contract.
- Browser and Edge currently contain separate implementations of the five radar scores. The previous parity test only searched both source files for similar expressions; it did not execute one shared model or prove equivalent input payloads.
- The visible profile for `Javiererd90` shows `100/77/25/100/0`, while the embedded social PNG shows an older polygon with low Fiabilidad.
- Card URLs only include `profileRevision`. A code-only scoring or renderer change does not increment that data revision, leaving the URL unchanged.
...
8. Keep profile data revisions and renderer revisions separate: data changes invalidate through `profileRevision`; formula/layout changes invalidate through the renderer revision.
...
- **Cache compatibility:** existing crawler caches cannot be purged retroactively, but the new renderer revision produces a different image URL as soon as the updated share document or browser bundle is fetched.
