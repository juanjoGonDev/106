...
- `game_attempt_integrity_decision()` currently gives three near-perfect attempts only 10 points for `sameDeviceNearPerfect >= 3` plus 10 for `sessionNearPerfect2h >= 3`. It does not score the stronger combination where the whole two-hour sample is near-perfect and at least two attempts are `<= 2 ms`.
...
Keep policy-v3 automatic exclusion gates unchanged, but strengthen its review score using evidence fields the engine already owns:

- add 25 risk points when a two-hour strong-identity sample has at least 3 attempts, at least 3 near-perfect attempts, and at least 2 very-near (`<=2 ms`) attempts;
- add 15 risk points when a two-hour sample of at least 3 attempts is entirely near-perfect (`<=5 ms`);
...
The observed three-attempt pattern `1 ms, 2 ms, 3 ms` on one account/device therefore rises from 20/100 to 60/100 (10 long-window frequency + 10 two-hour frequency + 25 extreme burst + 15 all-near), becoming `watch` while still requiring corroboration or human review for exclusion.
...
- [ ] New score branches are added to the real PostgreSQL policy matrix.
...
- Real PostgreSQL/Supabase:
  ...
  - risk scoring boundaries for extreme-burst/all-near signals, score cap, 1/2/3 regression and precision-only non-conviction.
...
- Increasing timing risk can create more false review positives. Mitigation: the new evidence only raises `watch` risk and cannot make `malicious=true` by itself.
